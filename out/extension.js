...
class DotNetDependencyAnalyzer {
    ...
    async analyzeSolution(solutionPath) {
        this.projects = [];
        this.nodes = [];
        this.edges = [];
    ...
    async scanDirectoryForProjects(directory) {
        try {
    ...
    extractProjectPaths(solutionContent, solutionDir) {
    ...
    extractPackageReferences(projectXml, project) {
        try {
    ...
    extractProjectReferences(projectXml, project, projectDir) {
        try {
            const itemGroups = projectXml.Project?.ItemGroup || [];
            for (const group of Array.isArray(itemGroups) ? itemGroups : [itemGroups]) {
    ...
    async extractAssemblyReferences(projectXml, project, projectDir) {
        try {
            const itemGroups = projectXml.Project?.ItemGroup || [];
            for (const group of Array.isArray(itemGroups) ? itemGroups : [itemGroups]) {
                if (group.Reference) {
    ...
    async scanForCompiledDependencies(projectDir, project) {
        try {
    ...
    async scanDirectoryForDlls(directory, project) {
        try {
            const entries = await readdir(directory, { withFileTypes: true });
            for (const entry of entries) {
                const fullPath = path.join(directory, entry.name);
...
function activate(context) {
    ...
    const disposable = vscode.commands.registerCommand('dotnetDependencyGraph.analyze', async (uri) => {
        ...
        try {
            ...
            if (uri) {
                ...
                if (solutionFiles.length === 0) {
                    ...
                    const selected = await vscode.window.showQuickPick(solutionFiles.map(f => ({ label: path.basename(f), detail: f })), { placeHolder: 'Select solution to analyze' });
                    if (!selected) {
                        return;
                    }
...
async function showDependencyGraph(context, graphData, solutionPath) {
    const panel = vscode.window.createWebviewPanel('dotnetDependencyGraph', '.NET Dependency Graph', vscode.ViewColumn.One, {
        enableScripts: true,
...
async function analyzePackageSecurity(graphData) {
    const analysis = {
        vulnerable: [],
        outdated: [],
        deprecated: [],
    ...
    for (const project of graphData.projects) {
        for (const pkg of project.packageReferences) {
            ...
            if (knownVulnerable.some((vuln) => packageId.includes(vuln))) {
                packageInfo.issues.push('Known security vulnerability');
                analysis.vulnerable.push(packageInfo);
            }
            else if (deprecated.some((dep) => pkg.name.includes(dep))) {
...
function getWebviewContent(graphData, solutionName, securityAnalysis) {
    ...
    return projects.map(project => `
        <div class="project-section">
            ...
            <table class="dependencies-table">
                ...
                <tbody>
                    ${project.packageReferences.map((pkg) => `
                        <tr>
                            <td>📦 NuGet</td>
                    ...
                    `).join('')}
                    ${project.projectReferences.map((ref) => `
                        <tr>
                            <td>🔗 Project</td>
...
function generateSecurityAnalysis(analysis) {
    return `
        <div class="project-section">
            <div class="project-header">
